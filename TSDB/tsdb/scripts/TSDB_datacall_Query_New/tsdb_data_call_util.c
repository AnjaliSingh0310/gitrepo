...
{  
  ...
  int sub_num = ns_get_random_number_int(1, 99);
...
*/
  ...
  int metric_num = ns_get_random_number_int(1, 99);
  ...
  if(tsdb_dc_ptr->metrics_data_mode == 0)
  {
   //tsdb_dc_ptr->mt_filter_option 0 -> Nofilter ,1 -> top N , 2 -> Buttom N 
    if (tsdb_dc_ptr->mt_filter_option == 0)
     tsdb_dc_ptr->data_filter = DF_MIN | DF_MAX | DF_AVG | DF_COUNT | DF_DATA_SUMMARY | DF_PARTIAL_SAMPLE ;
    else if (tsdb_dc_ptr->mt_filter_option == 1)
     tsdb_dc_ptr->data_filter = DF_MIN | DF_MAX | DF_AVG | DF_COUNT | DF_DATA_SUMMARY | DF_PARTIAL_SAMPLE | DF_MT_FILTER ; // Data filters
  }
...
void tsdb_data_query_set_tx_name(TSDBDataCall *tsdb_dc_ptr ,int *tx_time)
{
  ...
  if(tsdb_dc_ptr->time_period == 0)
  {
    ...
    {
      sprintf(ctp, "TPI%ds", tsdb_dc_ptr->incremental_freq); // currently we are  using only 1 min incremental call in custom time  
    }
    else
    { 
