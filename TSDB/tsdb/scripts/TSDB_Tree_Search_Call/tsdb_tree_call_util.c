...
{  
  ...
  tsdb_dc_ptr->rep_match_count = 0;
  int max_time_period = atoi(ns_eval_string("{MAX_TIME_PERIOD}"));
  tsdb_dc_ptr->time_period = 2;// ns_get_random_number_int(1, max_time_period);
  tsdb_dc_ptr->req_search_mode = atoi(ns_eval_string("{Search_mode}"));
  tsdb_dc_ptr->req_search_level = atoi(ns_eval_string("{Search_Level}"));
  if(tsdb_dc_ptr->time_period == 0) // Custom
  {
    // fill start and end ts
...
void tsdb_tree_search_query_set_tx_name(TSDBDataCall *tsdb_dc_ptr ,int *tx_time)
{
...
/*  char *tx_name_ptr = tsdb_dc_ptr->tx_name;
  tx_name_ptr += sprintf(tx_name_ptr, "Tree_Search_Call_TP%s_M%d_SL%d",                          
                                (tsdb_dc_ptr->time_period == 0)? ctp: time_period[tsdb_dc_ptr->time_period],
                                tsdb_dc_ptr->req_search_mode, tsdb_dc_ptr->req_search_level);

  if(tsdb_dc_ptr->rep_error_code == 0) // Success
  {
      tx_name_ptr += sprintf(tx_name_ptr, "_C%d_TS%d", tsdb_dc_ptr->rep_match_count, tsdb_dc_ptr->tree_size);
  } else {
    sprintf(tx_name_ptr, "_Error_%d", tsdb_dc_ptr->rep_error_code);

  }
  tsdb_tx_time = ns_get_transaction_time(TSDB_TCS_TX_NAME);
