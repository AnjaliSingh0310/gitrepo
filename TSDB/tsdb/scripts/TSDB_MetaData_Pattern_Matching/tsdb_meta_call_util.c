...
void tsdb_meta_data_query_init(TSDBMetaCall *tsdb_dc_ptr)
{
  char file_path[512];
...
void set_req_tp(TSDBMetaCall *tsdb_dc_ptr)
{

   int max_time_period = atoi(ns_eval_string("{MAX_TIME_PERIOD}"));
   tsdb_dc_ptr->time_period = ns_get_random_number_int(1,max_time_period);
   if(tsdb_dc_ptr->time_period == 0) /// Custom
    ...
    tsdb_dc_ptr->custom_duration = custom_duration[ns_get_random_number_int(4, max_custom_duration)]; // Take  from scen (values can  be .1, .2, ....1, 2, ...)
...
void tsdb_query_by_meta_data_set_req_fields(TSDBMetaCall *tsdb_dc_ptr)
{  
  char *field[10];

  // Set the subject mode as exact ,all ,pattern  
  ...
  tsdb_dc_ptr->req_freq = 1;  // 0 means request frequency will be ignored by TSDB
  ...
  // Set the metric filter - 0 - Disable , 1= Enable  
  ...
  if (tsdb_dc_ptr->mt_filter_option == 0)
     tsdb_dc_ptr->data_filter = 0;
  else if (tsdb_dc_ptr->mt_filter_option == 1)
     tsdb_dc_ptr->data_filter = DF_MT_FILTER ; // metric filters
  
  if(tsdb_dc_ptr->subject_mode == 0)
   tsdb_dc_ptr->req_num_metrics = 1;//ns_get_random_number_int(1, 5);
  else
   tsdb_dc_ptr->req_num_metrics = 1; // TODO
...
void tsdb_meta_data_open_merge_set_tx_name(TSDBMetaCall *tsdb_dc_ptr ,int *tx_time)
{
  ...
  if(tsdb_dc_ptr->time_period == 0)
  {
    ...
    {
      sprintf(ctp, "TPI1m"); // currently we are  using only 1 min incremental call in custom time  
    }
    else
    { 
 ...
 { 
  ...
  tx_name_ptr += sprintf(tx_name_ptr, "MetaData_OpenMerge_Call_M%d-%s_%s_H%d_DF%c_MF%d-%d-%d", 
  ...
  {
      tx_name_ptr += sprintf(tx_name_ptr, "MC%s", get_mc_sc_range(tsdb_dc_ptr->rep_num_metrics));
  } else {
  ...
  tsdb_tx_time = ns_get_transaction_time(TSDB_MD_OM_TX_NAME);
...
void tsdb_meta_data_open_related_set_tx_name(TSDBMetaCall *tsdb_dc_ptr ,int *tx_time)
{
  char ctp[8]; //Make custom TP
  int tsdb_tx_time;
  char nf_buffer[128] = "";
 ...
 else
 {
  ...
  tx_name_ptr += sprintf(tx_name_ptr, "MetaData_OpenRelatedCall_M%d-%s_%s_MF%d-%d-%d",
                                tsdb_dc_ptr->subject_mode, get_mc_sc_range(tsdb_dc_ptr->req_num_metrics),ctp,
                                tsdb_dc_ptr->mt_filter_option, tsdb_dc_ptr->metric_filter_type, tsdb_dc_ptr->mt_op);
  ...
  tsdb_tx_time = ns_get_transaction_time(TSDB_MD_OR_TX_NAME);
  *tx_time = tsdb_tx_time;
...
void tsdb_meta_data_pattern_matching_set_tx_name(TSDBMetaCall *tsdb_dc_ptr ,int *tx_time)
{
  char ctp[8]; //Make custom TP
  int tsdb_tx_time;
  char nf_buffer[128] = "";
 ...
 else
 {
  ...
  char *tx_name_ptr = tsdb_dc_ptr->tx_name;
  tx_name_ptr += sprintf(tx_name_ptr, "MetaData_Pattern_Matching_M%d-%s_%s_H%d_DF%c_MF%d-%d-%d",
                                tsdb_dc_ptr->subject_mode, get_mc_sc_range(tsdb_dc_ptr->req_num_metrics),ctp,
                                tsdb_dc_ptr->hierachy_levels, get_df_mode(tsdb_dc_ptr->metrics_data_mode), tsdb_dc_ptr->mt_filter_option, tsdb_dc_ptr->metric_filter_type, tsdb_dc_ptr->mt_op);
