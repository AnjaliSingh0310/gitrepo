...
int fill_metric_data_baseline(void *ptr)
{
  char metric[] = "Metric";
  int metric_len = strlen(metric) + 1;
  char metric_value[256];
  strcpy(metric_value, ns_eval_string("{metric_bl}"));
  int gdf_mt_id = atoi(ns_eval_string("{Graph_ID_bl}")); 
  ...
  metric_val_len = strlen(metric_value) + 1;
...
int fill_subject_or_measure(void* ptr, char* path, int gdf_mg_id, int flag, int flag2, TSDBMetaCall *tsdb_dc_ptr, int flag3)
{
  ...
  for(int i = 0; i < no_tokens; i++)
  {
     ...
     *val = '\0';
     ...
     if((flag == SUBJECT) && (i == (no_tokens - 0)) && (tsdb_dc_ptr->subject_mode == SUBJECT_MODE_ALL))
       val = "All";
     else if((flag== SUBJECT) && (i == (no_tokens - 1)) && (tsdb_dc_ptr->subject_mode == SUBJECT_MODE_PATTERN))
       val = "*";
     }
      /*
     //Added to make last subject value as All and last -1 value as '*' to test Exact/All/Pattern with same data file
...
int fill_metric_related_data(void *ptr)
{
  char metric[] = "Metric";
  int metric_len = strlen(metric) + 1;
  int mt_count_re = atoi(ns_eval_string("{METRIC_COUNT}"));
  char metric_value[1024 * 5];
  strcpy(metric_value,ns_eval_string("{metric}"));
  char *fields [190];
  uint8_t no_tokens = get_tokens_with_multi_delimiter(metric_value, fields, ":|",  190);  
 ...
 // memcpy(ptr, &no_tokens, 1);
  ...
  total_sz += 1;
  #ifdef TSDB_QUERY_LOGGING
    fprintf(query_log_fp, "\n no_tokens : %d ,mt_count: %d and %d-%d-%d\n", no_tokens, mt_count, atoi(fields[1]), atoi(fields[3]), atoi(fields[5]));
  #endif 
  for(int i = 0; i < mt_count; i++)
  {
 //    metric_val_len = strlen(ns_eval_string("{metric}")) + 1;
    metric_val_len = strlen(fields[i*2]) + 1;
    #ifdef TSDB_QUERY_LOGGING
    fprintf(query_log_fp, "\nvalue of metric_len: %d, metric: %s and id is %d\n", metric_val_len, fields[i*2], atoi(fields[(i*2)+1]));
    #endif
    memcpy(ptr, &metric_val_len, 1);
    ptr += 1;
    total_sz += 1;
...
//    memcpy(ptr, ns_eval_string("{metric}"), metric_val_len);  //TODO:dump metrics value after using token for ":"
    memcpy(ptr, fields[i*2], metric_val_len);  //TODO:dump metrics value after using token for ":"
    ptr += metric_val_len;
    total_sz += metric_val_len;

    uint32_t id = atoi(fields[(i*2)+1]);
    memcpy(ptr, &id, 4);
    ptr += 4;
    total_sz += 4;
  }
...
int create_req_for_meta_data_pattern_matching_call(TSDBMetaCall *tsdb_dc_ptr)
{
  ...
  int total_sz = sizeof(TSDB_Req_Metadata_Pattern_Match_t);

  uint8_t count = 3;

  TSDB_Req_Metadata_Pattern_Match_t *req = (TSDB_Req_Metadata_Pattern_Match_t *) tsdb_dc_ptr->buffer;
  req->msg_hdr.opcode = TSDB_QUERY_REQ_METADATA_PATTERN_MATCH;
  ...
  req->flag = 0;
  req->num_metric_set = atoi(ns_eval_string("{NUM_METRIC_SET}"));
  req->p_threshold = atoi(ns_eval_string("{P_THRESHOLD}"));
  
  #ifdef TSDB_QUERY_LOGGING
  fprintf(query_log_fp,"p_threshold = %d\n", req->p_threshold);
  #endif
  ...
  //Baseline Subject & Measure
  char baseline_subject[256];
  char baseline_measure[256];
  ns_advance_param("subject_bl");
  strcpy(baseline_subject, ns_eval_string("{subject_bl}"));
  strcpy(baseline_measure, ns_eval_string("{measure_bl}"));
  int gdf_mg_id_bl = atoi(ns_eval_string("{GDF_ID_bl}"));

  #ifdef TSDB_QUERY_LOGGING 
  fprintf(query_log_fp, "subject_bl = %s\n , measure_bl = %s and MG id %d\n",  baseline_subject, baseline_measure, gdf_mg_id_bl);
  #endif
  //filling subject.
  int size = fill_subject_or_measure(ptr, baseline_subject, 0, SUBJECT, 0, tsdb_dc_ptr, 0);
  total_sz += size;  
  ptr += size;
  //filling measure.
  *(uint8_t *)ptr = 3;  //measure mem count
  ptr++;
  size = fill_subject_or_measure(ptr, baseline_measure, gdf_mg_id_bl, MEASURE, 1, tsdb_dc_ptr, 0);
  total_sz += size;   
  ptr += size;

  //filling data of Metric.(As there can be multiple metric names and metric IDs.)
  size =  fill_metric_data_baseline(ptr); /*fill_metric_data(ptr);*/
  total_sz += size;
  ptr += size;

  //memcpy(ptr, &show_together, 1);
  //ptr += 1;
  //total_sz += 1;

  for(uint8_t i = 0; i < req->num_metric_set; i++)
  {
    //Relative Subject & Measure
    char relative_subject[256];
    char relative_measure[256];
    ns_advance_param("subject");
    strcpy(relative_subject, ns_eval_string("{subject}"));
    strcpy(relative_measure, ns_eval_string("{measure}"));
    int gdf_mg_id_re = atoi(ns_eval_string("{GDF_ID_Re}"));
    //filling subject.
    //memcpy(ptr, &count, 1);
    int size = fill_subject_or_measure(ptr, relative_subject, 0, SUBJECT, 0, tsdb_dc_ptr, 1);
    total_sz += size;
    ptr += size;
    //filling measure.
    memcpy(ptr, &count, 1);  //measure mem count
    ptr += 1;
    size = fill_subject_or_measure(ptr, relative_measure, gdf_mg_id_re, MEASURE, 1, tsdb_dc_ptr, 1);
    total_sz += size;
    ptr += size;
...
void tsdb_parse_response(void  *resp_buffer, int msg_size, TSDBMetaCall *tsdb_dc_ptr)
{
   ...
   TSDB_Resp_Metadata_t *resp = (TSDB_Resp_Metadata_t *)(resp_buffer);
    ...
    #ifdef TSDB_QUERY_LOGGING
    printf("Msg hdr. size: %d, opcode: %d\n", resp->msg_hdr.msg_size, resp->msg_hdr.opcode);
    printf("RT Hdr. qhead_rt: %d, tsdb_nodes: %d\n", resp->rt_hdr.qhead_rt, resp->rt_hdr.tsdb_nodes);
    printf("num_groups: %d\n", resp->mt_count);
    #endif
