...
void get_nd_agent_details(char *app_name, char *nd_app_server_host, char *tier_name, char *tier_tag, char *bci_start_time, char *bci_pid, char *bci_timestamp, char *server_ip, char *hostname, char *last_connection_time_stamp)
{
  int userID = ns_get_userid();
  int nvmID = ns_get_nvmid(); 
  int sessionID = ns_get_sessid();    
  ...
  sprintf(tier_tag, "%s", ns_eval_string("{Tag}"));
}
...
void  make_monitor_data(gdf_data *gdf_data_struct, int total_gdf_entries, char *buffer, gdf_file_struct **gdf_file, int idx, int tier_id, int instance_id, char *tier_name, char *nd_app_server_host, char *app_name, char *tier_tag )
{
  ...
  if(gdf_file_struct_temp->slot_used)
  {
    if(gdf_file_struct_temp->monitor_type == CUSTOM_MONITOR)
      get_custom_monitor_data(gdf_file_struct_temp, buffer, total_gdf_entries, idx, tier_id, instance_id, tier_name, nd_app_server_host, app_name, tier_tag);
    else if(gdf_file_struct_temp->monitor_type == DVM_MONITOR)
      get_dvm_monitor_data(gdf_file_struct_temp, buffer, total_gdf_entries, idx, tier_id, instance_id, tier_name, nd_app_server_host, app_name, tier_tag);
  }
 if(debug_level >= 2)
...
void get_dvm_monitor_data (gdf_file_struct *gdf_file_temp, char *outBuff, int total_gdf_entries, int idx, int tier_id, int instance_id, char *tier_name, char *nd_app_server_host, char *app_name, char *tier_tag)
{
  int i;
  int len = 0;
  ...
  {  
    ...
    len += sprintf (outBuff + len, "%d,%d|%d|%d:%s>%s>%s>%s_%d|%s|", gdf_file_temp->eventId, tier_id, instance_id, i , tier_name, nd_app_server_host, app_name, gdf_file_temp->vec_name, i, tier_tag);
...
void get_custom_monitor_data (gdf_file_struct *gdf_file_temp, char *outBuff, int total_gdf_entries, int idx, int tier_id, int instance_id, char *tier_name, char *nd_app_server_host, char *app_name, char *tier_tag)
{
  int len = 0;
  float random_val = 0.0f;
  ...
  { 
    ...
    len += sprintf(outBuff, "%d,%d|%d|%d:%s>%s>%s|%s|", gdf_file_temp->eventId, tier_id, instance_id, idx, tier_name, nd_app_server_host, app_name,tier_tag);
