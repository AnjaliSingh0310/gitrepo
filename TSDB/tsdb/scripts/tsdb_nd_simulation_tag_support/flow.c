...
int send_control_conn_request_to_ndc(char *app_name, char *nd_app_server_host, char *tier_name, char *tier_tag, char *bci_start_time, char *bci_timestamp, char *bci_pid, char *server_ip, char *hostname, char *last_connection_time_stamp)
{
  int  ret_val = -1;

  char ndagent_req[1024 + 1];

  if(debug_level >= 1)
  fprintf(stderr, "**** app_name = [%s], nd_app_server_host = [%s], tier_name = [%s], tier_tag = [%s], bci_start_time = [%s], bci_pid = [%s], bci_timestamp = [%s], server_ip = [%s], hostname = [%s], last_connection_time_stamp = [%s]\n", app_name, nd_app_server_host, tier_name, tier_tag, bci_start_time, bci_pid, bci_timestamp, server_ip, hostname, last_connection_time_stamp);
  
  sprintf(ndagent_req, "nd_ctrl_msg_req:appName=%s;ndAppServerHost=%s;"
                    "tierName=%s;bciVersion= VERSION 4.2.0 BUILD 102;bciStartTime=%s;ndHome=/home/netdiagnostics;pid=%s;attributes=%s"
                    ...
                    "connDestination=;\n",
                    app_name, nd_app_server_host, tier_name, bci_start_time, bci_pid, tier_tag, bci_timestamp, 
...
int send_monitor_data_to_ndc_on_data_conn(char *data)
{
  ...
  ns_save_string(data, "ndagent2ndc");
...
void flow()
{
  ...
  char gdf_file_path[] = "/home/cavisson/TSDB_NS/workspace/admin/default/cavisson/default/default/scripts/tsdb_nd_simulation/GDFConfig.txt.seq";
  ...
  get_nd_agent_details(app_name, nd_app_server_host, tier_name, tier_tag, bci_start_time, bci_timestamp, bci_pid, server_ip, hostname, last_connection_time_stamp);
  ...
  ret = send_control_conn_request_to_ndc(app_name, nd_app_server_host, tier_name, tier_tag, bci_start_time, bci_timestamp, bci_pid, server_ip, hostname, last_connection_time_stamp);  
  ...
  while(1)
  {
    if(send_monitor_data == 0)
    {
      while(1)
      {
        ...
        {
          parse_start_instrumentation_message(buffer, resp_size, &tier_id, &server_id, &instance_id, nd_app_server_host);
          send_monitor_data=1;
        }
    ...
    if(send_monitor_data == 1)
    {
      ...
      if(!last_time_data_sent || ((curr_time - last_time_data_sent) >= data_send_interval))
      {
        ...
        {
          memset(buffer, 0, BUFF_SIZE);
          make_monitor_data(gdf_data_struct, total_gdf_entries, buffer, &gdf_file, idx, tier_id, instance_id, tier_name, nd_app_server_host, app_name, tier_tag);
    ...
    if(ns_is_rampdown_user()==1)
    {
            printf("value of rampdown method = %d",ns_is_rampdown_user());
            goto err;
    }
  }//While 
