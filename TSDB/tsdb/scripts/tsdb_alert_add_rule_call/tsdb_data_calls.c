...
void tsdb_data_calls()
{
 ...
 //while(!ns_is_rampdown_user())
  {    
      ...
      int num_test_case = 1; //atoi(ns_eval_string("{Num_Test_Case}")); 
      for( int i=0; i < num_test_case; i++)
      { 
	  tsdb_query_by_name_set_req_fields(&tsdb_data_call);
	  int msg_len = create_req_for_alert_data_call(&tsdb_data_call, i);
...
//	#ifdef NS_DEBUG_ON
	...
	//#ifdef NS_DEBUG_ON
	  ...
	  //tsdb_alert_add_rule_query_set_tx_name(&tsdb_data_call, &tsdb_tx_time); // set tx name
	  ns_end_transaction_as(TSDB_DC_TX_NAME, NS_AUTO_STATUS, tsdb_dc_ptr->tx_name);
       
       sprintf(tsdb_dc_ptr->tx_name, "AlertAddRule");
       ...
       memset(tsdb_dc_ptr->tx_name, 0, 256);
     }
   }	  
