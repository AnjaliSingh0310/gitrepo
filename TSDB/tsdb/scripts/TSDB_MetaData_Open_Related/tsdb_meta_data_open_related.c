...
char metric_value; 
void tsdb_meta_data_open_related()
{
   ...
   while(!ns_is_rampdown_user())
  {
    ...
    int msg_len = create_req_for_meta_data_open_related_call(&tsdb_meta_call);
    ...
    ns_start_transaction(TSDB_MD_OR_TX_NAME);
   ...
   if(ret != 0)
   {
     ns_end_transaction_as(TSDB_MD_OR_TX_NAME, NS_AUTO_STATUS, "Meta_Data_Open_Related_Call_SendErr");
   ...
   if(ret != 0)
   {
      ns_end_transaction_as(TSDB_MD_OR_TX_NAME, NS_AUTO_STATUS, "Meta_Data_Open_Related_Call_RecvErr");
     goto close_connection;
   }
  ...
  // We need to send after parse of response as tx name will depend on the response 
   tsdb_meta_data_open_related_set_tx_name(&tsdb_meta_call, &tsdb_tx_time); // set tx name
   ns_end_transaction_as(TSDB_MD_OR_TX_NAME, NS_AUTO_STATUS, tsdb_dc_ptr->tx_name);
