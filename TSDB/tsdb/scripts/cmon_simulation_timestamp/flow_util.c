...
void get_server_ip_name(char *server_ip, char *server_name, char *tier_name, char *cmon_pid, char *tier_tag)
{
  ...
  //sprintf(server_name, "%s-%d-%d-0", tier_name, nvmID, userID); //TODO: Add generator ID in place of 0
...
void get_dvm_monitor_data (gdf_data *gdf_data_struct_tmp, char *outBuff, int *len)
{
  ...
  {  
    *len += sprintf(outBuff + *len, "%ld:%d:%s%d|", gdf_data_struct_tmp->mon_id, i, gdf_data_struct_tmp->vec_prefix, i);
...
void get_custom_monitor_data (gdf_data *gdf_data_struct_tmp, char *outBuff, int *len)
{
  ...
  *len += sprintf(outBuff + *len, "%ld:", gdf_data_struct_tmp->mon_id);
...
int fill_gdf_data(gdf_data **gdf_data_struct, char *gdf_name, long mon_id, int *total_gdf_entries, int *max_gdf_entries, int frequency)
{
  int idx;
  int row_num = 0;
  ...
  for(idx = 0; idx < *total_gdf_entries; idx++)
  {
   ...
   if(debug_level >= 2)
      fprintf(stdout, "MON ID already have %ld space, Ignoring idx [%d], GDF = %s\n", gdf_data_struct_tmp->mon_id , idx, gdf_name);
      return 1;
    }
  }
...
int parse_hb_response_message(char *recd_msg, int recd_msg_size, gdf_data **gdf_data_struct, int *total_gdf_entries, int *max_gdf_entries, int *testidx)
{
  ...
  if(debug_level >= 1)
    fprintf(stderr, "Monitor id = %ld , frequnecy = %d\n", mon_id, frequency);
