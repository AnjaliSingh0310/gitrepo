...
int send_control_conn_request_to_ndc(char *server_ip, char *server_name, char *tier_name, char *cmon_version, char *tier_tag)
{
  int  ret_val = -1;
...
//  if(debug_level >= 1)
  ...
  sprintf(cmon_req, "cmon_ctrl_msg_req:Server=%s;tierName=%s;tags=%s;"
                    ...
                    "connDuration=0;connDestination=NA;supportedProtocols=TCP;cmon_env_lmd=1545049161000\n",
                     server_name, tier_name, tier_tag, cmon_version, server_ip, cmon_pid);
...
//function for closing data connection
void close_data_conn()
{
  ns_start_transaction("Close_DataConnection");
  ns_web_websocket_close("ID=4",
"CODE=1000",
"REASON=Any reson");
  ns_end_transaction("Close_DataConnection", NS_AUTO_STATUS);
}

//function for closing control connection
void close_control_conn()
{
  ns_start_transaction("Close_ControlConnection");
  ns_web_websocket_close("ID=3",
"CODE=1000",
"REASON=Any reson");
  ns_end_transaction("Close_ControlConnection", NS_AUTO_STATUS);
}
...
void flow()
{
 ...
 // int data_send_interval = 60000;//In ms /TODO fill from cm_init request and for every monitor it will be differnt
  ...
  get_server_ip_name(server_ip, server_name, tier_name, cmon_version , tier_tag);
  ...
  ret = send_control_conn_request_to_ndc(server_ip, server_name, tier_name, cmon_version, tier_tag);  
  ...
  while(1)
  {
   ...
   // printf(" send_msg_to_ndc_on_control_connection failed ");
    ...
    // this loop will break once there is nothing to read on control connection
    while(1)
    {
	...
	//if cmt is not running then close data connection
        if(ret == 2)
        {
          close_data_conn();
        }
    ...
    //break loop if rampdown is done
