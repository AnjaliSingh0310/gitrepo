...
void tsdb_query_by_metric_init(TSDBDataCall *tsdb_dc_ptr)
{
  ...
  fprintf(stderr, "TSDB HOST: %s\n", ns_eval_string("{TSDB_HOST}")); 
...
{  
  ...
  tsdb_dc_ptr->time_period = tp_rn;
   ...
   char type_arr[][128] = {"","Tier","Tier>Server","Tier>Server>Instance","abcd"};
  tsdb_dc_ptr->type_rn = 0;//ns_get_random_number_int(0,1);
  strcpy(tsdb_dc_ptr->type,type_arr[tsdb_dc_ptr->type_rn]);
  ...
  if(tsdb_dc_ptr->time_period == 0) /// Custom
  {
    ...
    tsdb_dc_ptr->custom_duration = custom_duration[ns_get_random_number_int(6, max_custom_duration)]; // Take  from scen (values can  be .1, .2, ....1, 2, ...)
...
void tsdb_metric_group_query_set_tx_name(TSDBDataCall *tsdb_dc_ptr ,int *tx_time)
{
  ...
  tx_name_ptr += sprintf(tx_name_ptr, "Metric_Group_Call_TP%s_T%d",                          
...
void tsdb_metric_query_set_tx_name(TSDBDataCall *tsdb_dc_ptr ,int *tx_time)
{
  ...
  char *tx_name_ptr = tsdb_dc_ptr->tx_name;
  tx_name_ptr += sprintf(tx_name_ptr, "Metric_Graph_Call_TP%s_T%d",                          
                                (tsdb_dc_ptr->time_period == 0)? ctp: time_period[tsdb_dc_ptr->time_period],
                                tsdb_dc_ptr->type_rn);
