...
int create_req_for_metric_group_call(TSDBDataCall *tsdb_dc_ptr)
{
  ...
  int total_size = sizeof(TSDB_Req_Group_t) + app_id_len + client_id_len + total_type_len + 1 ;

  TSDB_Req_Group_t *req = (TSDB_Req_Group_t *)tsdb_dc_ptr->buffer;

  req->msg_hdr.version = 1;
  req->msg_hdr.opcode = TSDB_QUERY_REQ_GROUP;
...
int create_req_for_graph_call(TSDBDataCall *tsdb_dc_ptr)
{
  ...
  uint8_t app_id_len = strlen(app_id) + 1;
  uint8_t client_id_len = strlen(client_id) + 1;
  int num_groups = 1; //tsdb_dc_ptr->rep_num_mg;
     
  int total_size = sizeof(TSDB_Req_Graph_V2_t) + app_id_len + client_id_len + sizeof(g_metric_grp_id_t) * num_groups;

  TSDB_Req_Graph_V2_t *req = (TSDB_Req_Graph_V2_t *)tsdb_dc_ptr->buffer;

  req->msg_hdr.version = 1;
  req->msg_hdr.opcode = TSDB_QUERY_REQ_GRAPH_V2;
  req->msg_hdr.msg_size = total_size - 4;
  req->start_time = time(NULL) - 600; //tsdb_dc_ptr->start_ts; 
  req->end_time = time(NULL); //tsdb_dc_ptr->end_ts;
  req->app_id_len = app_id_len;
  req->client_id_len = client_id_len;
  req->preset = 0; //tsdb_dc_ptr->time_period;
  req->tr_num = 1234;
  req->num_groups = num_groups;
  #ifdef TSDB_QUERY_LOGGING  
   fprintf(query_log_fp,"\nreq->num_groups : %d\n, req->preset : %d \n, req->msg_hdr.msg_size : %d\n", req->num_groups, req->preset, req->msg_hdr);
   fflush(query_log_fp);
  #endif
 ...
 /* int rn_mg_id = ns_get_random_number_int(0,tsdb_dc_ptr->mg_id->g_mg_idx);
  ...
  {
   g_metric_grp_id_t *g_mg_id = var_data;
   var_data += 4;
...
void parse_group_resp(void *resp_buffer, TSDBDataCall *tsdb_dc_ptr)
{
  ...
  for(int j = 0; j < resp->count; ++j)
  {
    ...
    var_data += *length;
    #ifdef TSDB_QUERY_LOGGING 
    fprintf(query_log_fp,"mg_id: %d, id_type: %d, group_id: %d, name: %s, type: %s, is_group_vector: %d\n",
...
{
  TSDB_Resp_Graph_V2_t *resp = (TSDB_Resp_Graph_V2_t *)(resp_buffer);
  tsdb_dc_ptr->rep_tsdb_rt = resp->rt_hdr.qhead_rt;
  char rep_tx_name[1024];
  int tx_name_offset = 0;
  ...
  void *tx_ptr = (TSDB_Resp_Graph_V2_t *)resp_buffer;
  tx_ptr +=tx_name_offset;
  ...
  uint16_t *length = (uint16_t *)var_data;
  var_data += 2;
  memcpy(graph_desc, var_data, *length);
  graph_desc[*length] = '\0'; 
  var_data += *length;
...
void tsdb_parse_response(void  *resp_buffer, int msg_size, TSDBDataCall *tsdb_dc_ptr)
{
  ...
  switch(hdr->opcode)
  {
    case TSDB_QUERY_REQ_GROUP:
      parse_group_resp(resp_buffer, tsdb_dc_ptr);  
      break; 
    case TSDB_QUERY_REQ_GRAPH:
