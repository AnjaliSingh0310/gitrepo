...
void get_server_ip_name(char *server_ip, char *server_name, char *tier_name, char *cmon_pid, char *tier_tag)
{
  int userID = ns_get_userid();
  int nvmID = ns_get_nvmid(); 
  int sessionID = ns_get_sessid();
  grp_name = strdup(ns_eval_string("{cav_sgroup_name}"));

  fprintf(stderr, "**********************Group Name ************* - %s\n", grp_name);

  
//if (!strcmp(grp_name,"G1"))
  if (userID < 1000)
  {
	  sprintf(tier_name,"Fixed-cav-tier");
	  sprintf(server_name,"Fixed-server-%d",userID);
  }
  else
  {
...
int fill_gdf_data2(gdf_data **gdf_data_struct, char *gdf_name, int mon_id, int *total_gdf_entries, int *max_gdf_entries, int frequency)
{
  int idx;
  int row_num = 0;
  gdf_data *gdf_data_struct_tmp = *gdf_data_struct;
  ...
  ns_save_string(gdf_name,"DV_GDFName2");
  ns_eval_string("{DV_GDFName2}");
  if(!strcmp("", ns_eval_string("{NumberOfElement2}")))
  {
    fprintf(stderr, "GDF %s not found in index param, Will not send data for this monitor.\n", gdf_name);
    return 1;
  }
  ...
  gdf_data_struct_tmp = *gdf_data_struct + row_num;  
  ns_save_string(gdf_name,"DV_GDFName2");
  strcpy(gdf_data_struct_tmp->gdf_name, ns_eval_string("{DV_GDFName2}"));
  gdf_data_struct_tmp->numOfElement = atoi(ns_eval_string("{NumberOfElement2}"));
  gdf_data_struct_tmp->monitor_type = atoi(ns_eval_string("{MonitorType2}"));
  gdf_data_struct_tmp->addInterval =  atoi(ns_eval_string("{AddInterVal2}"));
  gdf_data_struct_tmp->delInterval =  atoi(ns_eval_string("{DeleteInterval2}"));
  gdf_data_struct_tmp->minVectors = atoi(ns_eval_string("{MinVectors2}"));
  gdf_data_struct_tmp->maxVectors = atoi(ns_eval_string("{maxVectors2}"));
  strcpy(gdf_data_struct_tmp->vec_prefix, ns_eval_string("{VectorPrefix2}"));
  char *field[460]; 
  char *read_buf = ns_eval_string("{Data2}");
  int no_tokens = get_tokens_with_multi_delimiter(read_buf, field, ":|", 460);
  for(int i=0; i<gdf_data_struct_tmp->numOfElement; i++)
  {
    gdf_data_struct_tmp->maxData[i] = atof(field[i*2]);
...
int parse_hb_response_message(char *recd_msg, int recd_msg_size, gdf_data **gdf_data_struct, int *total_gdf_entries, int *max_gdf_entries, int *testidx)
{
  ...
  //Check if cmt is running or not
  if(!strncmp(decoded_recd_msg, "cmon_get_config_msg_resp:type=5;", 32))
  {
    return 2;
  }
  ...
  *ptr2 = '\0'; 
  ...
  strcpy(grp_name, ns_eval_string("{cav_sgroup_name}"));
  int ret;
  int userID = ns_get_userid();
  if (userID < 1000) 
     ret = fill_gdf_data2(gdf_data_struct, gdf_name, mon_id, total_gdf_entries, max_gdf_entries, frequency);
  else
     ret = fill_gdf_data(gdf_data_struct, gdf_name, mon_id, total_gdf_entries, max_gdf_entries, frequency);
