...
@pytest.fixture
def json_file_path():
    return "/home/cavisson/work/TSDB/tsdb/automation/testsuites/alerts/alert_ch_json.json"

@pytest.mark.parametrize(
    "ch_type",
    ChangeType,
    ids=[str(ch.value) for ch in ChangeType],
)
@pytest.mark.parametrize(
    "metric_att",
    MetricAttribute,
...
def test_parameterized_condition_values(severity, operand, operator, Window, timeWindow, subtype, metric_att, ch_type, json_file_path):
    """
    ...
    with open(json_file_path, "r+") as f:
        ...
        data["rules"][0]["name"] = f"automation_test_ch_{i}"
        i = i + 1 
        ...
        if ch_type.value == "1":
            data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["changeType"]["change"] = ch_type.value
        ...
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["changeType"]["dataType"] = operand.value
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["changeType"]["operator"] = operator.value
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["changeType"]["windowType"] = Window.value
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["changeType"]["timeWindow"] = timeWindow.value
