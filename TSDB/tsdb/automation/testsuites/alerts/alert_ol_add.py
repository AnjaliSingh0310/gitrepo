...
@pytest.fixture
def json_file_path():
    return "/home/cavisson/work/TSDB/tsdb/automation/testsuites/alerts/alert_ol_json.json"
...
def test_parameterized_condition_values(severity, operand, Window, algotype, timeWindow, subtype, metric_att, json_file_path):
    """
    Tests with all combinations of severity, operand, values.

    Creates a copy of the provided JSON file for each test case,
    updates the `dataType` values, and prints a success message.
    """

    # Create a copy of the original JSON file with a unique suffix
    #new_file_path = f"{json_file_path}_{severity}_{operand.value}_{Window.value}_{algotype.value}_{timeWindow.value}_{subtype.value}.json"
    ...
    with open(json_file_path, "r+") as f:
        ...
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["outliersType"]["dataType"] = operand.value
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["outliersType"]["windowType"] = Window.value
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["outliersType"]["algoType"] = algotype.value
        data["rules"][0]["attributes"]["severity"][0]["conditionList"][0]["outliersType"]["timeWindow"] = timeWindow.value
